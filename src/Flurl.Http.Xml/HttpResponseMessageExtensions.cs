...
public static class IFlurlResponseExtensions
{
    ...
    private static async Task<T> ReceiveFromXmlStream<T>(this Task<IFlurlResponse> response, Func<Stream, T> streamHandler)
    {
        var resp = await ReceiveXmlResponseMessage(response).ConfigureAwait(false);
        try
        {
        ...
        catch (Exception ex)
        {
            ...
            throw new FlurlHttpException(null, s, ex);
        }
    }
    ...
    public static async Task<T> ReceiveXml<T>(this Task<IFlurlResponse> response) => await ReceiveFromXmlStream(response, stm => MicrosoftXmlSerializer.Default.Deserialize<T>(stm));
    ...
    {
        return await ReceiveFromXmlStream(response, stm =>
        {
